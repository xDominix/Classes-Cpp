...
{
public:
    Vector(size_t s):data_((Fraction*)malloc(s*sizeof(Fraction))),size_(0),capacity_(s){};
    Vector():Vector(0){};
    Vector(const Vector&v);
    ~Vector(){delete [] data_;};

    size_t size() const noexcept{ return size_;};//const??
    size_t capacity() const noexcept{ return capacity_;};
    Fraction* data() const noexcept{ return data_;};

    Vector& operator=(const Vector& v);

    Fraction operator[](size_t i){if(i<size_) return data_[i]; else throw out_of_range( "Index out of range" );};
    const Fraction operator[](size_t i) const {if(i<size_) return data_[i]; else throw out_of_range( "Index out of range" );};
